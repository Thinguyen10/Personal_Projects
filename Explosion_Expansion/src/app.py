    ...
    with st.spinner("Analyzing (this runs once, then displays results)…"):
        ...
        with torch.inference_mode():
            for v, a in zip(vids, mels):
                # ----- VIDEO PREPROCESS -----
                # v shape: [C,H,W], could be 9 channels (3 frames × 3 RGB channels)
                if v.shape[0] == 9:
                    # Collapse 9 channels → 3 channels by averaging frames
                    v = v.view(3, 3, v.shape[1], v.shape[2]).mean(dim=0)  # [3,H,W]
                # Ensure batch dimension
                v = v.unsqueeze(0)  # [1,3,H,W]

                # ----- AUDIO PREPROCESS -----
                # a shape: [M,T] or [1,M,T]
                if a.ndim == 3:
